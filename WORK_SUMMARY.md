...
## 2026-04-09 增量更新（图生视频上传链路）

### 本次新增
- 视频实验室新增图生上传区交互增强：拖拽上传、进度条、删除已选图片。
- 新增后端图生上传接口：`POST /api/video/upload-image`。
- 新增七牛 Kodo 上传能力：本地图上传后自动转为公网 URL 返回给前端。

### 本次修复
- 修复图生参考图切换页面后丢失：后端项目状态持久化补充 `image_url/start_image_url/end_image_url`。
- 修复 Kodo 上传机房不匹配：出现 `incorrect region` 时自动按提示域名重试。
- 增强错误可观测性：上传失败时展示七牛返回的详细错误内容。
